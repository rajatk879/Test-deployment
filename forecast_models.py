...
class StatsModelWrapper:
    ...
    def predict(self, future):
        ...
        forecast_values = []
        current_value = self.last_value  # Start from last known value
        current_level = level
        
        for i in range(n_periods):
            # Update level using exponential smoothing
            current_level = alpha * current_value + (1 - alpha) * current_level
            # Add trend component
            forecast_value = current_level + trend * (i + 1)
...
class SimpleModel:
    ...
    def predict(self, future):
        ...
        # Use deterministic seed based on first date to ensure reproducibility
...
class MC4ForecastModel:
    ...
    def prepare_data(self, df, sku_id=None):
        ...
        # This ensures we have one value per date for training
        df_agg = df.groupby("date")["forecast_tons"].sum().reset_index()
        df_agg.columns = ["ds", "y"]
    ...
    def forecast(self, sku_id, periods=30, start_date=None):
        ...
        # If start_date is provided, calculate how many periods we need from model's last date
        if start_date:
            ...
            if model_last_date and start_date_ts > model_last_date:
                # Calculate periods needed from model's last date to cover requested range
                ...
                # Start date is before or equal to model's last date, generate from last date
